...
if (isset($_POST['update_product'])) {
    ...
    $conn->query("UPDATE `products` SET name = '$name', details = '$details', price = '$price' WHERE id = '$update_p_id'");
    ...
    if (!empty($image)) {
        if ($image_size > 2000000) {
            ...
            $conn->query("UPDATE `products` SET image = '$image' WHERE id = '$update_p_id'");
...
<?php
   ...
   $select_products = $conn->query("SELECT * FROM `products` WHERE id = '$update_id'");
